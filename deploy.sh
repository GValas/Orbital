...
#   NAS_PATH  dossier servi par nginx sur le NAS (défaut : /mnt/user/appdata/orbital/site)
...
NAS_PATH="${NAS_PATH:-/mnt/user/appdata/orbital/site}"
NAS_PORT="${NAS_PORT:-22}"

DRY=""
[ "${1:-}" = "--dry-run" ] && DRY="--dry-run"

echo "==> Build de production (build.sh)…"
./build.sh
...
# rsync : --delete pour refléter exactement dist/ (supprime les vieux artefacts)
